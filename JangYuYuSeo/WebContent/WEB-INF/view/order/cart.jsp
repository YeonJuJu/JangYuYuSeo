...
<script type="text/javascript">
	function update_amount(id) {
		...
		form.setAttribute('action',
				"/JangYuYuSeo/cart/update_amount?cart_product_id=" + id
						+ "&amount=" + number);
	...
	function change_qty2(type, id) {
		...
		// 결과 출력
		result.value = number;
		total.innerText = (number * price.innerText).toLocaleString('ko-KR', {
			style : 'currency',
			currency : 'KRW'
		});
	}
								...
								<form:form id="check" method="get" action="${root}cart/orderform">
									<c:forEach var="product" items="${cartProductList}">
										<tr class="alert" role="alert">
											...
											<td id='total${product.cart_pr_id}'>₩<fmt:formatNumber value="${product.total_price}" pattern="#,###,###" /></td>
											
											<td class="border-bottom-0">
												<button type="button" class="close"
													onClick="location.href='${root }cart/cartProductDelete?cart_product_id=${product.cart_pr_id}'">
