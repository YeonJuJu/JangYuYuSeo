...
<head>
    ...
    <link rel="stylesheet" href="${root}resources/summernote/css/summernote-lite.css">
    
    <script src="${root}resources/summernote/js/summernote-lite.js"></script>
	<script src="${root}resources/summernote/js/lang/summernote-ko-KR.js"></script>
...
function uploadSummernoteImageFile(file, editor) {	
	...
	$.ajax({
		...
		success : function(data) {
			$(editor).summernote('editor.insertImage', ${root}+data.url);
		}
	});
}
</script>
	...
	<div class="limiter">
		<div class="container-login100">
			<div class="wrap-login100">
				...
				<form:form
					action="${root }product/modify?product_id=${productDTO.pr_id}"
						...
						<form:select path="pr_category">
							<c:forEach var="category" items="${categoryList}" begin="2">
								<option value=${category.category_idx
									} ${category.category_idx == productDTO.pr_category ? 'selected="selected"':''}>${category.category_name}</option>
							</c:forEach>
						</form:select>
					</div>
					...
					<div class="wrap-input100 validate-input m-b-26">
						<span class="label-input100">대표 이미지</span> <input type="file"
							id="gdsImg" name="file" /> <br></br>
					...
					<div class="wrap-input100 validate-input m-b-26">
						...
						<form:input path="pr_description" class="input100"
							placeholder="상품설명 입력" value="${productDTO.pr_description}" />
						<span class="focus-input100"></span>
					...
					<div class="wrap-input100 validate-input m-b-26">
						...
						<form:input path="pr_size" class="input100"
							placeholder="상품사이즈 입력(,로 구분하여 입력)" value="${productDTO.pr_size}" />
						<span class="focus-input100"></span>
					...
					<div class="wrap-input100 validate-input m-b-26">
						...
						<form:input path="pr_color" class="input100"
							placeholder="상품색상 입력(,로 구분하여 입력)" value="${productDTO.pr_color}" />
						<span class="focus-input100"></span>
					...
					<div class="wrap-input100 validate-input m-b-26">
						...
						<form:input path="pr_stock" class="input100" placeholder="상품재고 입력"
							value="${productDTO.pr_stock}" />
						<span class="focus-input100"></span>
						<form:errors path="pr_stock" style="color:red;" />
					</div>
					<div class="wrap-input100 validate-input m-b-26">
						<span class="label-input100">상세설명</span>
						<textarea id="summernote" name="pr_detail">${productDTO.pr_detail}</textarea>
					</div>
