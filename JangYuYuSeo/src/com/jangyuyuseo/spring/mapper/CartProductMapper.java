...
public interface CartProductMapper {
	@Insert("INSERT INTO CART_PRODUCT_TABLE "
			...
			+ "#{pr_amount}, #{pr_color}, #{pr_size},#{total_price}, 'false')")
	public void addCartProduct(CartProductDTO cartProductDTO);
	//카트에 같은 상품이 있는지 찾는 용도
	@Select("Select cart_pr_id, cart_id, pr_id, pr_amount, pr_color, pr_size, total_price, decode(is_checked, 'true', 'true', 'false', 'false') from cart_product_table where cart_id=#{cart_id} and pr_id=#{pr_id} and pr_color=#{pr_color} and pr_size=#{pr_size}")
	...
	@Select("select cart_pr_id, pr_id, pr_amount, pr_color, pr_size from cart_product_table where cart_id=#{cart_id} and is_checked='true'")
